...
watch(
    ...
    (open) => {
        ...
        form.first_registration_date = props.vehicle?.first_registration_date ?? '';
...
<template>
    <AppModal :open="open" :title="isEditMode ? 'Fahrzeug bearbeiten' : 'Neues Fahrzeug anlegen'"
        ...
        <form @submit.prevent="submit">
            <div class="grid grid-cols-1 gap-x-6 gap-y-3 px-2 md:grid-cols-2">
                ...
                <LicensePlateInput v-model="form.license_plate" :disabled="isEditMode"
                    :server-error="form.errors.license_plate" />
                ...
                <FormField v-slot="{ id, describedBy, invalid }" label="Marke" required :error="form.errors.make">
                    <SearchableSelectField :id="id" v-model="form.make" :options="VEHICLE_BRAND_OPTIONS"
                        placeholder="Marke wählen" search-placeholder="Marke suchen..."
                        empty-label="Keine Marke gefunden" :invalid="invalid" :described-by="describedBy" />
                </FormField>

                <FormField v-slot="{ id, describedBy, invalid }" label="Modell" :error="form.errors.model">
                    <Input :id="id" v-model="form.model" placeholder="Modell eingeben" autocomplete="off"
                        :aria-invalid="invalid" :aria-describedby="describedBy" />
                </FormField>

                <div>
                    <FormField v-slot="{ id, describedBy, invalid }" label="Leasingende"
                        :error="form.errors.leasing_end_date">
                        <CalendarDateField :id="id" v-model="form.leasing_end_date" allow-past
                            :disabled="leasingEndUnknown" :invalid="invalid" :described-by="describedBy" />
                            <Label :for="leasingEndUnknownId" class="mt-1.5 flex cursor-pointer items-start gap-2 font-normal">
                                <Checkbox :id="leasingEndUnknownId" v-model="leasingEndUnknown"
                                ...
                                <span class="text-xs leading-[1.45] font-normal text-[#00000099]">
                                    Das genaue Datum des Leasingendes liegt mir aktuell nicht vor. Ich werde Ihnen diese
                                    Information zeitnah nachreichen.
                                </span>
                            </Label>
                        <CalendarDateField v-model="form.first_registration_date" label="Erstzulassung" allow-past
                            :allow-future="false" :error="form.errors.first_registration_date" />
                    </FormField>
                </div>

                <div>
                    <FormField v-slot="{ id, describedBy, invalid }" label="Leasinggeber" required
                        :error="form.errors.leasinggeber">
                        <Input :id="id" v-model="form.leasinggeber" placeholder="Leasinggeber eingeben"
                            autocomplete="off" :disabled="leasinggeberUnknown" :aria-invalid="invalid"
                            :aria-describedby="describedBy" />
                    </FormField>
                    <Label :for="leasinggeberUnknownId"
                        class="mt-1.5 flex cursor-pointer items-start gap-2 font-normal">
                        <Checkbox :id="leasinggeberUnknownId" v-model="leasinggeberUnknown"
                            class="mt-0.5 size-4 shrink-0 rounded-[4px] border-gray-300 data-[state=checked]:border-emerald-500 data-[state=checked]:bg-emerald-500" />
                        <span class="text-xs leading-[1.45] font-normal text-[#00000099]">
                            Der Name des Leasinggebers liegt mir aktuell nicht vor. Ich werde Ihnen diese Information
                            zeitnah nachreichen.
                        </span>
                    </Label>
                </div>
                ...
                <template v-if="showFleetFields">
                    <div class="mt-2 flex items-center gap-3 md:col-span-2">
                        <span
                            class="text-[10.5px] font-bold tracking-[0.16em] text-[#9CB3B4] uppercase">Flottendaten</span>
                        <span class="h-px flex-1 bg-[#EDF2F2]" />
                    </div>

                    <FormField v-slot="{ id, describedBy, invalid }" label="Kilometerstand"
                        :error="form.errors.mileage">
                        <Input :id="id" v-model="form.mileage" type="number" min="0" placeholder="z. B. 45000"
                            :aria-invalid="invalid" :aria-describedby="describedBy" />
                    </FormField>

                    <FormField v-slot="{ id, describedBy, invalid }" label="Vertragsnummer"
                        :error="form.errors.contract_number">
                        <Input :id="id" v-model="form.contract_number" maxlength="100"
                            placeholder="Vertragsnummer eingeben" :aria-invalid="invalid"
                            :aria-describedby="describedBy" />
                    </FormField>

                    <FormField v-slot="{ id, describedBy, invalid }" label="Kostenstelle"
                        :error="form.errors.cost_centre">
                        <Input :id="id" v-model="form.cost_centre" maxlength="100" placeholder="Kostenstelle eingeben"
                            :aria-invalid="invalid" :aria-describedby="describedBy" />
                    </FormField>

                    <FormField v-slot="{ id, describedBy, invalid }" label="Fahrer / Ansprechpartner"
                        :error="form.errors.driver_name">
                        <Input :id="id" v-model="form.driver_name" maxlength="255" placeholder="Name eingeben"
                            :aria-invalid="invalid" :aria-describedby="describedBy" />
                    </FormField>

                    <FormField v-slot="{ id, describedBy, invalid }" label="Kontakt des Fahrers"
                        label-hint="(E-Mail oder Telefon)" :error="form.errors.driver_contact" class="md:col-span-2">
                        <Input :id="id" v-model="form.driver_contact" maxlength="255"
                            placeholder="E-Mail oder Telefonnummer" :aria-invalid="invalid"
                            :aria-describedby="describedBy" />
                    </FormField>

                    <div class="mt-2 flex items-center gap-3 md:col-span-2">
                        <span
                            class="text-[10.5px] font-bold tracking-[0.16em] text-[#9CB3B4] uppercase">Abholadresse</span>
                        <span class="h-px flex-1 bg-[#EDF2F2]" />
                    </div>

                    <FormField v-slot="{ id, describedBy, invalid }" label="Straße"
                        :error="form.errors['collection_address.street']">
                        <Input :id="id" v-model="form.collection_address.street" maxlength="255"
                            placeholder="Straße eingeben" :aria-invalid="invalid" :aria-describedby="describedBy" />
                    </FormField>

                    <FormField v-slot="{ id, describedBy, invalid }" label="Hausnummer"
                        :error="form.errors['collection_address.number']">
                        <Input :id="id" v-model="form.collection_address.number" maxlength="50" placeholder="Nr."
                            :aria-invalid="invalid" :aria-describedby="describedBy" />
                    </FormField>

                    <FormField v-slot="{ id, describedBy, invalid }" label="Adresszusatz"
                        :error="form.errors['collection_address.additional_address']" class="md:col-span-2">
                        <Input :id="id" v-model="form.collection_address.additional_address" maxlength="255"
                            placeholder="Halle, Tor, Etage …" :aria-invalid="invalid" :aria-describedby="describedBy" />
                    </FormField>

                    <FormField v-slot="{ id, describedBy, invalid }" label="PLZ"
                        :error="form.errors['collection_address.zip_code']">
                        <Input :id="id" v-model="form.collection_address.zip_code" maxlength="20" placeholder="PLZ"
                            :aria-invalid="invalid" :aria-describedby="describedBy" />
                    </FormField>

                    <FormField v-slot="{ id, describedBy, invalid }" label="Ort"
                        :error="form.errors['collection_address.city']">
                        <Input :id="id" v-model="form.collection_address.city" maxlength="100" placeholder="Ort"
                            :aria-invalid="invalid" :aria-describedby="describedBy" />
                    </FormField>

                    <FormField v-slot="{ id, describedBy, invalid }" label="Land"
                        :error="form.errors['collection_address.country']" class="md:col-span-2">
                        <Input :id="id" v-model="form.collection_address.country" maxlength="100"
                            placeholder="Deutschland" :aria-invalid="invalid" :aria-describedby="describedBy" />
                    </FormField>
                </template>
            </div>
        </form>
