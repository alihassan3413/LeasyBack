...
const navByRole: Record<UserType, NavItem[]> = {
    ...
    Admin: [
        { label: 'Dashboard', icon: MdiViewDashboardOutline, name: 'admin.dashboard' },
        { label: 'Mein Konto', icon: MdiAccountOutline, name: 'profile.edit' },
    ],
};
  ...
  -->
    ...
    -->
        ...
        <nav class="flex min-h-0 flex-1 flex-col gap-0.5 overflow-x-hidden overflow-y-auto pr-0.5">
            ...
            >
                <span v-if="item.icon" class="flex shrink-0 items-center">
            ...
            >
                ...
                <transition name="lb-fade">
                    <div v-if="!collapsed" class="min-w-0 flex-1 overflow-hidden">
                        <div class="truncate text-[12.5px] font-bold text-white">
                            {{ userName }}
                        </div>
