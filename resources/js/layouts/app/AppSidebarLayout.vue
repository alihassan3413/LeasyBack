    ...
    <div class="flex h-dvh flex-col overflow-hidden md:gap-4 md:p-4">
        ...
        <div class="relative hidden flex-1 overflow-hidden md:flex">
            ...
            <main class="flex min-w-0 flex-1 flex-col overflow-hidden bg-white">
                ...
                <div class="min-w-0 flex-1 overflow-x-hidden overflow-y-auto p-6">
                    <slot />
                </div>
            </main>
        </div>
        ...
        <div class="relative flex flex-1 flex-col overflow-hidden md:hidden">
            ...
            <!--
                Bottom tab bar for mobile. The shell is `h-dvh`, so this row is
                pinned inside the *visible* viewport even while mobile Chrome's
                URL bar collapses/expands — with `h-screen` (100vh) it used to be
                pushed under the browser chrome and looked like it had vanished.
                Labels are shown so "Mein Konto"/"Abmelden" are discoverable.
            -->
            <nav
                class="z-50 flex shrink-0 items-center justify-around px-1 pt-2"
                style="
            ...
            >
                <button
                    ...
                    class="flex min-w-0 flex-1 flex-col items-center gap-0.5 rounded-lg px-1 py-1 transition-all"
                    :class="isActive(item.name) ? 'text-[#01B990]' : 'text-white/55 hover:text-white'"
                >
                    <component :is="item.icon" :style="{ width: '22px', height: '22px' }" />
                    <span class="max-w-full truncate text-[10px] leading-tight font-semibold">{{ item.label }}</span>
                </button>
                <button
                    @click="handleLogout"
                    class="flex min-w-0 flex-1 flex-col items-center gap-0.5 rounded-lg px-1 py-1 text-white/55 transition-all hover:text-white"
                >
                    <MdiLogout :style="{ width: '22px', height: '22px' }" />
                    <span class="max-w-full truncate text-[10px] leading-tight font-semibold">Abmelden</span>
                </button>
            </nav>
        </div>
    </div>
