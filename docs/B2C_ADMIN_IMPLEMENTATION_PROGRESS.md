...
**Checkpoint 9 — Admin customer management (this session)** — complete:
...
Everything the plan defers past Checkpoint 9, still untouched:
...
- **Live browser verification of the Checkpoint 3, 5, 7, 8, and 9 pages** — functionally proven via Feature tests + build/lint, but never actually clicked through. Do this before considering any fully signed off; prioritize the Vehicle modals (drag-and-drop, plate input, Select interactivity), `OrderCreationModal`'s date/time + station picker, the Admin sidebar's collapse/expand + disabled-item states, and the new Admin sidebar's reskinned colors (visually confirm against the legacy panel, not just the surveyed hex values), since they're the most interaction-heavy/visually-distinct new UI so far.
- The actual Fahrzeuge/Alle Aufträge Admin pages — `AdminLayout`/`AdminSidebar` exist and the nav shows them as disabled placeholders; Checkpoints 10/11 build the real pages behind them.
...
None functionally. One environment limitation carried over from Checkpoint 3: browser automation isn't available in this session, so none of the Inertia pages built in Checkpoints 3, 5, 7, 8, or 9 have been visually/interactively verified.

## 7. Exact next checkpoint

**Checkpoint 10 — Admin vehicle management**: Admin vehicle list/detail/create-on-behalf-of-customer; extend the already-reasonably-built vehicle report document transfer/upload/publish/delete flow (no rebuild needed there). Build the vehicle pages inside `AdminLayout`, replacing the "Fahrzeuge" sidebar item's disabled placeholder with a real link — `AdminQueryService::vehicles()` already exists and is reused by the Checkpoint 9 customer detail page, so this checkpoint's list page is largely wiring, not new query logic. Do not start Checkpoint 11 (Admin order management) until Checkpoint 10 is reviewed.

## 8. Handoff note for another Claude session

Checkpoints 1 through 9 are done and merged into the working tree (not committed to git unless you're told to). If you're picking this up cold:
